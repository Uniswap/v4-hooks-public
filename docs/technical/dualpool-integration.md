# DualPool: Integration Guide for Aggregators & Routers

**Audience:** third-party aggregators, routers, solvers, and quote APIs that want to
source liquidity from `DualPoolHook` pools.
...
1. **Detect** a DualPool pool: its `PoolKey.hooks` address returns `true` for
...
2. **Quote** using the `IALFHook` views — **never** the pool's on-chain liquidity.
   A DualPool holds **zero persistent v4 liquidity between swaps**; `getSlot0` shows a
...
## 2. Mental model: what a DualPool pool looks like from outside

A DualPool pool is a standard Uniswap v4 pool whose `hooks` address is a deployed
...
events, indexers, operator onboarding), then confirm the hook is a DualPool:
...
//    DualPool requires: beforeInitialize, beforeAddLiquidity, beforeRemoveLiquidity,
//    beforeSwap, afterSwap. All *ReturnDelta flags are false.
//    See getHookPermissions() — DualPoolHook.sol:600.
```

A pool that passes (1) is an ALF hook; DualPool is one ALF strategy among several
(spread quoters, etc.). You do not need to distinguish DualPool specifically — the
`IALFHook` surface is uniform. If you want to, DualPool-only signals include the
...
- **Exact input** → `amountSpecified < 0`. `getIndicativeQuote` returns the **expected
  output**. ([DualPoolHook.sol:562](../../src/alf/DualPoolHook.sol#L562))
...
DualPool is a static-pricing strategy. `beforeSwap`, `getIndicativeQuote`, and
`swapToPrice` **ignore `hookData` entirely** ([DualPoolHook.sol:67](../../src/alf/DualPoolHook.sol#L67)).
...
return `0` when the hook can't price the swap. DualPool returns `(0, …)` when:
...
See [`_simulateIndicative`](../../src/alf/DualPoolHook.sol#L955-L991). Treat a `0`
...
- From the `PoolKey` you already hold: `key.fee` (a concrete `uint24`, **never** the
  dynamic-fee flag `0x800000` — DualPool rejects dynamic-fee pools at init,
  [DualPoolHook.sol:298](../../src/alf/DualPoolHook.sol#L298)).
...
([`_composeEffectiveFee`](../../src/alf/DualPoolHook.sol#L993-L996)), so `getIndicativeQuote`
...
- `getEffectiveLiquidity` = **what the hook can withdraw and deploy this block**:
  ...
  ([DualPoolHook.sol:542-549](../../src/alf/DualPoolHook.sol#L542-L549)).
...
There is nothing DualPool-specific in the execution path. Swap as you would against
any v4 pool:
...
- **Native ETH is unsupported.** DualPool rejects `address(0)` currencies at init
  ([DualPoolHook.sol:293](../../src/alf/DualPoolHook.sol#L293)); pools are WETH-style.
  No `msg.value` path.
- **No callbacks into you.** DualPool returns `ZERO_DELTA` from `beforeSwap` and `0`
...
If you want onchain competitive execution across several pools (a DualPool plus other
...
MultiplexerHookData memory hd = MultiplexerHookData({
    attestationData: "",                     // ignored by DualPool candidates
    targets: [ TargetedQuoter({ poolKey: dualPoolKey, amountSpecified: 0 }), ... ],
    strictTolerancePips: 0                   // or a downside tolerance in ppm
});
poolManager.swap(multiplexerPoolKey, params, abi.encode(hd));
```

How DualPool behaves as a candidate:
...
- **Pre-planned mode** (any target carries a non-zero `amountSpecified`): you supply the
  ...
  order.
- A **paused DualPool reverts `PoolNotLive`**, which the multiplexer's per-target
...
> ([DualPoolHook.sol:363-377](../../src/alf/DualPoolHook.sol#L363-L377)), not at
...
price limit ([`_simulateIndicative`](../../src/alf/DualPoolHook.sol#L955-L991),
[`_activeIndicativeLiquidity`](../../src/alf/DualPoolHook.sol#L998-L1029)).
...
// DualPool-specific reads (src/alf/DualPoolHook.sol, DualPoolBase.sol)
