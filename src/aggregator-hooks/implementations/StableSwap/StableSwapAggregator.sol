...
contract StableSwapAggregator is BaseAggregatorHook {
    ...
    /// @notice Curve's address for native ETH (V4 uses address(0))
        ...
        returns (uint256 amountSettle, uint256 amountTake, bool hasSettled)
    {
        ...
        uint256 received = settleCurrency.balanceOfSelf() - balanceBefore;
        (amountSettle) = _settleAndMeasureActual(settleCurrency, received);
        hasSettled = true;
        return (amountSettle, amountTake, hasSettled);
    }

    /// @notice Settle to PoolManager and return the actual amount received (handles fee-on-transfer, rounding)
    /// @dev We must do this for tokens like stETH that can have rounding differences upon transfers
    function _settleAndMeasureActual(Currency settleCurrency, uint256 amountToSend)
        internal
        returns (uint256 actualSettled)
    {
        uint256 pmBefore = settleCurrency.balanceOf(address(poolManager));
        _settle(settleCurrency, address(this), amountToSend);
        actualSettled = settleCurrency.balanceOf(address(poolManager)) - pmBefore;
    }
}
