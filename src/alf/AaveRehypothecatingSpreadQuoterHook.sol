...
contract AaveRehypothecatingSpreadQuoterHook is SpreadQuoterBase, IUnlockCallback {
    ...
    /// @dev If the current tick is outside the LP range, remove and redeploy at the snapped tick.
    function _autoReposition(PoolKey calldata key) internal {
        ...
        // Reposition to adjacent tick range. Since external LP is blocked, the hook's
        // LP is the only liquidity — when exhausted, the tick may overshoot to MIN/MAX_TICK.
        // Deploying at the adjacent range keeps token amounts manageable and the LP near
        // the last traded price.
        int24 spacing = key.tickSpacing;
        int24 newLower;
        if (currentTick < pos.tickLower) {
            newLower = pos.tickLower - spacing;
        } else {
            newLower = pos.tickUpper;
        }
        int24 newUpper = newLower + spacing;
        ...
        // Net settle. Uses mint (claims) for positive deltas because during afterSwap
        // the swapper hasn't settled yet, so PoolManager may lack ERC-20 for _take.
        _settleNetDeltaViaClaims(key, removeDelta, addDelta);

        // Update state
        activeLowerTick[poolId] = newLower;
        activePosition[poolId] = ActivePosition(newLower, newUpper, pos.liquidity);

        emit ActiveTickUpdated(poolId, newLower);
        emit LiquidityRepositioned(poolId, newLower, newUpper, pos.liquidity);

        // Rebalance excess ERC-20 into Aave
    ...
    /// @dev Like _settleNetDelta but receives positive deltas as ERC-6909 claims instead of ERC-20.
    ///      Used in afterSwap where the PoolManager may not yet hold enough ERC-20 (swapper
    ///      hasn't settled).
    function _settleNetDeltaViaClaims(PoolKey calldata key, BalanceDelta removeDelta, BalanceDelta addDelta) internal {
        _settleOrMint(key.currency0, removeDelta.amount0() + addDelta.amount0());
        _settleOrMint(key.currency1, removeDelta.amount1() + addDelta.amount1());
    }

    /// @dev Settle (pay) or mint claims (receive) based on the sign of net.
    function _settleOrMint(Currency currency, int128 net) internal {
        if (net < 0) {
            uint256 amount = uint256(int256(-net));
            _ensureErc20(currency, amount);
            _settle(currency, address(this), amount);
        } else if (net > 0) {
            poolManager.mint(address(this), currency.toId(), uint256(int256(net)));
        }
    }
