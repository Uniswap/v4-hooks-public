...
/// @author Uniswap Labs
/// @notice `SmartPoolHook` composed with the `RewardsLib` liquidity-incentives capability. LP
///         shares earn a third reward token via Synthetix-style per-second accrual, on top of
///         the JIT spread quoting and ERC-4626 rehypothecation inherited unchanged from
///         `SmartPoolHook`.
///
...
///         ledger verbatim. It wires `MultiAssetVault._onShareCheckpoint` (which fires on
///         bootstrap / deposit / withdraw, before the share counts move) to
///         `RewardsLib.checkpoint`, so accrual settles exactly when LP positions change.
///
///         ## Trust model
///
///         Adds one capability to the operator surface: funding rewards. The owner binds a
///         reward token ({setRewardToken}, permanent), sets the period cadence
///         ({setRewardsDuration}), and funds periods ({notifyRewardAmount}, pulling the reward
///         token from the caller). The reward token MUST NOT be either pool currency, so reward
///         custody never aliases the pool's ERC-20 inventory tracked by `InventoryLib`. The
...
contract SmartPoolIncentivizedHook is SmartPoolHook {
    ...
    using RewardsLib for RewardsLib.Rewards;
    using SafeERC20 for IERC20;

    /// @dev `notifyRewardAmount` was called before a reward token was bound via
    ///      {setRewardToken}.
    error RewardTokenNotConfigured();
    /// @dev `setRewardToken` was given one of the pool's own currencies. The reward token must
    ///      be distinct so reward custody never aliases `InventoryLib`'s per-pool ERC-20 ledger.
    ...
    function _onShareCheckpoint(VaultId vaultId, address user, uint256 totalSharesBefore, uint256 userSharesBefore)
        ...
        override
    {
        RewardsLib.load().checkpoint(vaultId, user, totalSharesBefore, userSharesBefore);
    }
    ...
    /// @notice Bind the reward token for a pool. Permanent (accrued balances must resolve
    ///         against one token). Rejects either pool currency.
    ...
    function setRewardToken(PoolKey calldata key, IERC20 token) external onlyOwner whenJITNotInProgress {
        ...
        RewardsLib.load().setRewardToken(_vaultIdFor(key.toId()), token);
    }
    ...
    function setRewardsDuration(PoolKey calldata key, uint256 duration) external onlyOwner whenJITNotInProgress {
        RewardsLib.load().setRewardsDuration(_vaultIdFor(key.toId()), duration);
    }
        ...
        whenJITNotInProgress
    {
        ...
        IERC20 token = RewardsLib.load().rewardTokenOf(id);
        if (address(token) == address(0)) revert RewardTokenNotConfigured();
        token.safeTransferFrom(msg.sender, address(this), reward);
        RewardsLib.load().notifyRewardAmount(id, reward, _totalShares[id]);
    }
    ...
    function claimRewards(PoolKey calldata key) external nonReentrant whenJITNotInProgress returns (uint256 amount) {
        VaultId id = _vaultIdFor(key.toId());
        amount = RewardsLib.load().claim(id, msg.sender, _totalShares[id], _userShares[id][msg.sender]);
    }
    ...
    function earned(PoolKey calldata key, address user) external view returns (uint256) {
        VaultId id = _vaultIdFor(key.toId());
        return RewardsLib.load().earned(id, user, _userShares[id][user], _totalShares[id]);
    }
}
