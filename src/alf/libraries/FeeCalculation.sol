...
/// @dev ALF copy of `src/stable/libraries/FeeCalculation.sol`, byte-identical body, consumed by the
///      `DecayingFee` capability. Kept separate so neither tree imports across the other. Not to be
///      confused with `FeeLib`: this library computes the dynamic LP fee itself, while
///      `FeeLib.effectiveSwapFee` composes an LP fee with v4's directional protocol fee on the
///      quote paths; the quote pipeline uses both.
