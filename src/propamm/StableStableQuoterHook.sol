...
/// @notice IQuoterHook-compliant dynamic fee hook for stable/stable pairs, integrating the
///         StableStableHook's reference-price fee model with the PropAMM quoter framework.
///         Discoverable via PropAMMIndex, queryable via getIndicativeQuote, routable by the
///         auction hook. Supports EIP-712 signed curve updates for atomic fee config changes.
contract StableStableQuoterHook is BasePropAMMHook, FeeConfiguration, EIP712, Ownable2Step {
    ...
    /// @notice Initialize a pool with this hook and register it in the PropAMMIndex.
    ...
    function initializePool(
        ...
        _registerInIndex(poolKey, QuoterType.STORAGE, "");
    ...
    /// @notice Update fee configuration and PropAMM state for a pool.
