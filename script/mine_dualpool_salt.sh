#!/usr/bin/env bash
#
# mine_dualpool_salt.sh — find a CREATE2 salt for a DualPoolHook deployment whose address has BOTH
# the required v4 hook flags (address & 0x3fff == 0x2ac0) AND N leading zero hex digits, using the
# local create2crunch fork at ~/dev/create2crunch.
#
# Hooks deploy through the DualPool family's AllowlistedFactory (the CREATE2 deployer), so FACTORY is required and every
# salt is mined against THAT address: a salt mined for a different factory (or for the old direct
# 0x4e59 CREATE2-proxy path) resolves to a different address and fails the deploy script's checks.
# Deploy the factory first (script/DeployDualPoolFactory.s.sol; mine ITS vanity salt with
# script/mine_factory_salt.sh), then mine hook salts against it.
#
...
#
# DualPoolHook permissions -> flag bits:
#   beforeInitialize (0x2000) | beforeAddLiquidity (0x0800) | beforeRemoveLiquidity (0x0200)
#     | beforeSwap (0x0080) | afterSwap (0x0040)  ==  0x2AC0        (mask Hooks.ALL_HOOK_MASK = 0x3fff)
#
...
#
# The init-code hash (correctness anchor) is computed here from the CURRENT DualPoolHook creation
# bytecode plus the ABI-encoded constructor args, exactly as the deploy site will
# (`new DualPoolHook{salt}(pm, maxGas, owner, maxMinDepositBlocks)`). Change any constructor arg (or
# the contract) and the hash — and any previously-mined salt — is invalid. Every mined salt is
...
#   POOL_MANAGER, HOOK_OWNER (required); MAX_GAS (default 800000), MAX_MIN_DEPOSIT_BLOCKS (default 3600).
#
# Usage:
#   FACTORY=0x… POOL_MANAGER=0x… HOOK_OWNER=0x… [MAX_GAS=…] [MAX_MIN_DEPOSIT_BLOCKS=…] [MIN_LEADING_ZERO_NIBBLES=6] \
#   [MATCHES=0] [GPU_DEVICE=0] [USE_CPU=1] [BACKEND=] [CREATE2CRUNCH_DIR=~/dev/create2crunch] [CALLER=0x0…] \
#     ./script/mine_dualpool_salt.sh                 # build (rebuilds on source change), mine, verify each match
#
#   ./script/mine_dualpool_salt.sh params            # print the CREATE2 params (init-code hash, flags, etc.)
#   ./script/mine_dualpool_salt.sh verify <salt>     # recompute a salt's address and check the conditions
set -euo pipefail
...
# DualPoolHook's 14 permission bits (see header). FLAG_MASK is Hooks.ALL_HOOK_MASK.
FLAG_SUFFIX="2ac0"
FLAG_MASK="3fff"
...
# Hook constructor: (IPoolManager pm, uint32 maxGas_, address owner_, uint64 maxMinDepositBlocks_)
MAX_GAS="${MAX_GAS:-800000}"
MAX_MIN_DEPOSIT_BLOCKS="${MAX_MIN_DEPOSIT_BLOCKS:-3600}"
...
load_params() {
  ...
  need forge
  need cast
  [ -n "$CREATE2_DEPLOYER" ] || die "FACTORY is required (the AllowlistedFactory address; deploy it with script/DeployDualPoolFactory.s.sol)"
  [ -n "${POOL_MANAGER:-}" ] || die "POOL_MANAGER is required"
  [ -n "${HOOK_OWNER:-}" ] || die "HOOK_OWNER is required"
  ...
  args=$(cast abi-encode "constructor(address,uint32,address,uint64)" \
    "$POOL_MANAGER" "$MAX_GAS" "$HOOK_OWNER" "$MAX_MIN_DEPOSIT_BLOCKS") || die "cast abi-encode failed"
...
MAX_GAS=$MAX_GAS
MAX_MIN_DEPOSIT_BLOCKS=$MAX_MIN_DEPOSIT_BLOCKS
...
verify_salt() {
  ...
  echo "  factory.deploy(type($HOOK_NAME).creationCode, abi.encode($POOL_MANAGER, $MAX_GAS, $HOOK_OWNER, $MAX_MIN_DEPOSIT_BLOCKS), $salt)"
}
