...
/// @notice Port of `test/stable/libraries/FeeCalculation.t.sol` against the ALF copy of the
///         library. The library body is byte-identical to `src/stable`'s; identical expectations
///         here pin the copy, so any divergence is a copy error.
...
/// @notice Validates the ALF FeeCalculation copy against spreadsheet-derived test vectors.
/// @dev Port of `test/stable/data/FeeCalculationSpreadsheet.t.sol`; the vectors are copied to
///      `test/alf/data/` so the trees stay independent.
...
contract FeeCalculationSpreadsheetTest is Test {
    ...
    string constant DATA_PATH = "test/alf/data/fee_calculation_test_data.csv";
