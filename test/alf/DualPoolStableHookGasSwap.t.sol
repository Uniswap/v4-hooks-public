...
import {FeeConfig} from "../../src/alf/types/DecayingFee.sol";
import {LiquidityBucket} from "../../src/alf/types/Distribution.sol";
import {MockERC4626} from "./mocks/MockERC4626.sol";

/// @notice Focused gas guard for DualPoolStableHook's JIT swap path with the dynamic fee engine.
///         Mirror of `DualPoolHookGasSwapTest`; the target carries the fee engine's overhead
///         (config + state reads, the once-per-block checkpoint write, and the extra slot0 read)
///         on top of the static-fee baseline.
contract DualPoolStableHookGasSwapTest is Test, Deployers {
    ...
    uint256 constant SINGLE_BUCKET_VAULTED_SWAP_GAS_TARGET = 725_000;
    ...
    function test_gas_singleBucketVaultedSwap() public {
        ...
        uint256 gasUsed = vm.snapshotGasLastCall("DualPoolStableHook_swap_singleBucket_withVault");
    ...
    function _initPool(LiquidityBucket[] memory dist, bool withVault) internal returns (PoolKey memory key) {
        ...
        DualPoolStableHook.PoolConfig memory cfg = DualPoolStableHook.PoolConfig({
            ...
            feeConfig: FeeConfig({
                ...
                targetMultiplier: 50,
                referenceSqrtPriceX96: TickMath.getSqrtPriceAtTick(0)
            }),
