...
contract StableSwapNGFuzz is Test {
    ...
    /// @notice Fuzz test: Swap through V4 hook with random number of tokens (2-8)
    ...
    function testFuzz_v4SwapMultiToken(uint256 numTokensRaw, uint256 amplificationRaw, uint256 seed) public {
        ...
        ICurveStableSwapNG curvePool = ICurveStableSwapNG(curvePoolAddr);

        // Deploy V4 hook (creates all pair pools)
        Currency[] memory currencies = _toCurrencies(tokens);
        StableSwapNGAggregator hook = _deployHookMulti(curvePool, currencies);
