...
- **Ethereum mainnet (chain id 1):** **Fluid**, **StableSwap**, **StableSwap-NG**, **Uniswap V3** (`UniswapV3AggregatorForkTest`), **Uniswap V2** (`UniswapV2AggregatorForkTest`), and **LitePSM** (`LitePSMAggregatorForkTest`) use `FORK_RPC_URL_1` and optional `FORK_BLOCK_NUMBER_1` (0 = latest). The suite **skips** when `FORK_RPC_URL_1` is unset.
...
| Role          | Ethereum (Uni V3 fork)                                                                                        | Ethereum (Uni V2 fork)                                                                                                                                                                                | Ethereum (LitePSM fork)                                                                                                                                                                                           | Base (Slipstream fork)                                                                                                        | Base (Pancake V3 fork)                                                                                                                                                 |
| ------------- | ------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Factory       | `UNISWAP_V3_FACTORY` — `0x1F98431c8aD98523631ae4a59f267346ea31F984`                                           | V2 fork tests use `UNISWAP_V2_EXTERNAL_PAIR.factory()` (same chain as the pair); do not set mainnet `0x5c69…` on L2 / non-mainnet forks.                                                              | N/A — singleton; no factory lookup                                                                                                                                                                                | `SLIPSTREAM_FACTORY` — Slipstream **Pool factory** `0x5e7BB104d84c7CB9B682AaC2F3d509f5F406809A` (not the pool implementation) | `PANCAKE_V3_FACTORY` — `0x0BFbCF9fa4f9C56B0F40a671Ad40E0805A091865`                                                                                                    |
| External pool | `UNISWAP_V3_EXTERNAL_POOL` — e.g. WETH/USDT 0.3% `0x4e68Ccd3E89f51c3074ca5072bbac773960dFa36` (includes USDT) | `UNISWAP_V2_EXTERNAL_PAIR` — required when `FORK_RPC_URL_1` is set; e.g. WETH/USDT `0x0d4a11d5EEaaC28EC3F61d100daF4d40471f1852` (includes USDT). If unset while RPC is set, the fork suite **skips**. | `LITE_PSM_WRAPPER` — LitePSMWrapper address, e.g. USDC/USDS `0xA188EEc8F81263234dA3622A406892F3D630f98c`. `USDS` — USDS token `0xdC035D45d973E3EC169d2276DDab16f1e407384F`. Suite **skips** when either is unset. | `SLIPSTREAM_EXTERNAL_POOL` — any Slipstream pool, e.g. WETH/USDC `0xdbc6998296caA1652A810dc8D3BaF4A8294330f1`                 | `PANCAKE_V3_EXTERNAL_POOL` — any Pancake V3 pool on Base; USDT tests expect **bridged USDT** `0xfDE4C96C8593536E31F229Ea8f37B2adAbC26991` in the pair (see fork test). |
...
# LitePSM fork (`test/aggregator-hooks/LitePSM/`)
LITE_PSM_WRAPPER=
USDS=
```
