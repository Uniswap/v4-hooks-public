...
contract FeeConfigurationImplementationTest is Test {
    ...
    function test_updateFeeConfig_revertsWithNotConfigManager() public {
        FeeConfig memory newConfig = FeeConfig({
            k: K, logK: LOG_K, optimalFeeRateE6: OPTIMAL_FEE_RATE_E6, referenceSqrtPriceX96: REFERENCE_SQRT_PRICE_X96
        });
    ...
    function test_updateFeeConfig_revertsWithInvalidReferenceSqrtPriceX96() public {
        FeeConfig memory newConfig = FeeConfig({
            k: K, logK: LOG_K, optimalFeeRateE6: OPTIMAL_FEE_RATE_E6, referenceSqrtPriceX96: TickMath.MIN_SQRT_PRICE - 1
        });
    ...
    function test_updateFeeConfig_revertsWithInvalidOptimalFeeRateE6() public {
        FeeConfig memory newConfig =
            FeeConfig({k: K, logK: LOG_K, optimalFeeRateE6: 1e6, referenceSqrtPriceX96: REFERENCE_SQRT_PRICE_X96});

        vm.prank(poolFeeController);
        vm.expectRevert(abi.encodeWithSelector(IFeeConfiguration.InvalidOptimalFeeRateE6.selector, 1e6));
        feeConfigurationImplementation.updateFeeConfig(testPoolKey.toId(), newConfig);
    }
    ...
    function test_updateFeeConfig_succeeds() public {
        ...
        (k, logK, optimalFeeRateE6, referenceSqrtPriceX96) =
            feeConfigurationImplementation.feeConfig(testPoolKey.toId());
        assertEq(k, K);
        assertEq(logK, LOG_K);
